...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if emotion == "Neutral":
            ...
            cv2.putText(frame, message, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
...
if neutral_detections > 0:
    ...
    if total_emotions == 0:
        ...
        probabilities = {emotion: count / total_emotions for emotion, count in emotion_trends.items()}
        
        # Generate a weighted random choice for variability
        predicted_emotion = random.choices(list(probabilities.keys()), weights=probabilities.values(), k=1)[0]
        ...
        future_prediction = random.choice(prediction_options.get(predicted_emotion, ["Your emotions seem unpredictable. Stay mindful! 🧘"]))
